...
module.exports = (app) => (base, controller, validator) => {
  ...
  app.get(path.join(base, ':id'), validate(validator.get), handle(async (req, res) => {
    const { id } = req.params;
    res.status(status.OK).json(await controller.get(id))
  }));

  app.put(path.join(base, ':id'), validate(validator.update), handle(async (req, res) => {
    const { body } = req;
    res.status(status.OK).json(await controller.update(body));
  }));

  app.delete(path.join(base, ':id'), validate(validator.delete), handle(async (req, res) => {
    const { body } = req;
    res.status(status.OK).json(await controller.update(body));
  }));
}
